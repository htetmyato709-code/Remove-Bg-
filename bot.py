...
# Railway Variables မှ ဖတ်ယူပါသည်
BOT_TOKEN = os.environ.get("BOT_TOKEN")
OWNER_ID_RAW = os.environ.get("OWNER_ID")

if not BOT_TOKEN:
    raise ValueError("❌ BOT_TOKEN မတွေ့ပါ။ Railway Variables တွင် BOT_TOKEN ထည့်ပေးပါ။")

if not OWNER_ID_RAW:
    raise ValueError("❌ OWNER_ID မတွေ့ပါ။ Railway Variables တွင် OWNER_ID ထည့်ပေးပါ။")

OWNER_ID = int(OWNER_ID_RAW)
# -----------------------------------------------------------------------

# Database Setup (User ID များကို အမြဲသိမ်းဆည်းထားရန်)
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if current_mode == "rembg":
            ...
            processed_bytes = await loop.run_in_executor(None, remove, photo_bytes)
            caption = "✅ Background ဖျောက်လုပ်ဆောင်ချက် ပြီးပါပြီ။"
        elif current_mode == "hq":
            # Image Enhancement
            processed_bytes = await loop.run_in_executor(None, enhance_image_quality, bytes(photo_bytes))
            caption = "✅ Premium Quality မြှင့်တင်ပေးထားပါသည်။"
            
        await status_msg.delete()
        
        # Transparent background နှင့် Resolution မကျစေရန် Document အနေဖြင့် ပို့ပေးခြင်း
